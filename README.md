...
4. **访问应用**
   打开浏览器访问 http://localhost:3000 admin/admin
...
```
依赖：自动拉取node:18-alpine不成功时，需要手动拉取
```bash
docker pull swr.cn-north-4.myhuaweicloud.com/ddn-k8s/docker.io/node:18-alpine
docker tag  swr.cn-north-4.myhuaweicloud.com/ddn-k8s/docker.io/node:18-alpine  docker.io/node:18-alpine
```
...
# 正确输出
sop-manager-backend-1   | SOP Library后端服务已准备好！
sop-manager-backend-1   | 服务器运行在 http://localhost:3000
sop-manager-frontend-1  | 
sop-manager-frontend-1  | > sop-manager-app@0.0.0 dev
sop-manager-frontend-1  | > vite
